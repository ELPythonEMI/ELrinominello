...
## Immagini nel repository

Inserisci i due file immagine nella cartella principale del progetto, nello stesso livello del file `README.md`:
...
I nomi dei file devono corrispondere esattamente a:
...
Se il repository non è ancora stato creato, crealo su GitHub con il nome `ELrinominello`, quindi esegui:

```bash
git init
git add ELrinominello.py README.md elrinominello.png elrinominello1.png .gitignore
git commit -m "Prima versione di ELrinominello"
git branch -M main
git remote add origin https://github.com/ELPythonEMI/ELrinominello.git
git push -u origin main
```

Per aggiornare il progetto in futuro:
...
Puoi distribuire questo progetto con una licenza MIT. Prima della pubblicazione, aggiungi un file `LICENSE` con il testo ufficiale della licenza.
